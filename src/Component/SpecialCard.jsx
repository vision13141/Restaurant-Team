...
const SpecialCard = ({ item, heart = true }) => {
  const { AddcartPopUp, addToWish, love, add_remove_ToToast } = useContext(Context);
    ...
    <div className="w-[310px] h-[440px] bg-transparent rounded-md relative hover:-translate-y-3 duration-150 ease-linear hover:scale-[1.05] group">
      <div className="relative">
        ...
        {heart && (
          ...
          >
            {love.includes(id) ? <FaHeart onClick={() => add_remove_ToToast("Remove To Wishlist")} /> : <CiHeart onClick={() => add_remove_ToToast("Add To Wishlist")} />}
          </div>
        )}
      </div>
